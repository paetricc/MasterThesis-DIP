...
# Soubor: large_experiments_10000.sh
# Popis:
# Skript spouštějící experimenty pro
# všechny algoritmy na instanci 10000. 
...
for ((i=1; i<=EXPERIMENTS; i++)); do
    ...
    nohup nice $SCRIPT "qiga" $FILE_PATH --evaluations $EVAL_LIMIT --experiments 1 --population 1 --theta 0.002 --output $OUTPUT --append_results > "$LOG_DIR/qiga_${INSTANCE}_run${i}.log" 2>&1 &
